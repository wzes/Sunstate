            ...
            <el-dialog title="Update Contract Info" :visible.sync="dialogFormVisible">
                <el-form :model="selectTable">
                  <el-form-item label="User" label-width="100px">
                    <el-input v-model="selectTable.user.name" auto-complete="off"></el-input>
                  </el-form-item>
...
export default {
  ...
  created () {
    if (!this.$store.state.c_initialize) {
      for (let i = 0; i < 14; i++) {
        let contract = {
          user: {
            name: this.names[i]
        ...
        this.$store.state.contracts.push(contract)
        this.count = this.$store.state.contracts.length * 10
      }
      this.$store.state.c_initialize = true
    } else {
      this.currentPage = 1
      this.count = this.$store.state.contracts.length
    }
  ...
  methods: {
    ...
    handleEdit (index) {
      this.selectTable = this.$store.state.contracts[index]
      this.dialogFormVisible = true
    },
    async handleDelete (index) {
      try {
        if (this.count < 15) {
          this.$store.state.contracts.splice(index, 1)
        } else {
          const res = {status: 1}
          if (res.status === 1) {
            this.$message({
            ...
            let row = this.$store.state.contracts[index]
            this.$store.state.contracts.splice(index, 1)
            this.$store.state.contracts.push(row)
          } else {
